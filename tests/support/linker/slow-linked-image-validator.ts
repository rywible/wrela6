...
const SLOW_SECTION_ORDER = Object.freeze([
  ".text",
  ".rdata",
  ".data",
  ".pdata",
...
function slowPlacementFor(layout: AArch64LinkedImageLayout): SlowPlacement {
  const diagnostics: LinkerDiagnostic[] = [];
  const contributionByKey = new Map<string, SlowContributionPlacement>();
  ...
  for (const section of orderedSections) {
    ...
    if (section.rva !== expectedSectionRva) {
      diagnostics.push(
        diagnostic(
          `slow-image-layout:section-rva-mismatch:${section.stableKey}:${section.rva}:${expectedSectionRva}`,
        ),
      );
    }
    ...
    for (const contribution of [...section.contributions].sort(compareContributionsByPlacement)) {
      ...
      if (contribution.offsetBytes !== expectedContributionOffsetBytes) {
        diagnostics.push(
          diagnostic(
            `slow-image-layout:contribution-offset-mismatch:${contribution.stableKey}:${contribution.offsetBytes}:${expectedContributionOffsetBytes}`,
          ),
        );
      }
    ...
    if (section.virtualSizeBytes !== nextContributionOffset) {
      diagnostics.push(
        diagnostic(
          `slow-image-layout:section-size-mismatch:${section.stableKey}:${section.virtualSizeBytes}:${nextContributionOffset}`,
        ),
      );
    }
...
function slowWordToU32Le(bytes: readonly number[]): bigint {
...
function sectionPriority(sectionKey: string): number {
  const index = SLOW_SECTION_ORDER.indexOf(sectionKey);
