...
describe("W1-08a return-of-parameter inlining reproduction", () => {
  test("inlining an identity callee leaves every result use verifier-clean", () => {
    const call = sourceCall(10, "identity", [1], [20]);
    const callerUse = addOperation(11, 21, 20, 2);
    const program = programForTest([
      functionWithOperations({
        functionId: 1,
        instance: "caller",
        parameters: [optIrValueId(1), optIrValueId(2)],
    ...
    const result = inline(program, [call, callerUse]);
...
function inline(
  ...
  operations: Parameters<typeof runWholeProgramInliningForTest>[0]["operations"],
