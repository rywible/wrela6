...
Implementation completed on 2026-07-01 on the checked-out `aarch64-backend` branch.

Concrete findings folded back into the plan:

- The canonical backend pipeline now includes the Task 33 and Task 34 stages (`debug-artifact-collection` and `end-to-end-stage-wiring`) in addition to the Task 8 verifier shell. The public compile API keeps the compatibility verifier key `input-contract` while also exposing the canonical stage key `verify-input-contract`.
- `AArch64ObjectModule` remains the strict production constructor. Object-verifier negative tests that need corrupted or out-of-range final objects use object-like verifier inputs derived from valid modules instead of weakening production construction invariants.
- Byte provenance records carry optional `factFamilies` and `machineSubjectKey` metadata so Task 33 and Task 34 can explain fact-to-byte spending without embedding host paths, timestamps, environment data, or external-tool output.
- The layout-and-encode owner emits object-module-compatible sections, symbols, relocations, literal pools, veneers, byte provenance, and monotone branch/literal/veneer decisions together. Compile integration rewraps that output with the full verification summary and unwind/fact-spending records.
- Independent review found and the implementation corrected four late integration gaps: the public compile path now invokes the semantic backend stages before layout, unsupported machine opcodes fail closed instead of lowering to `nop`, fact import/input-contract checks enforce verifier and target-declaration authority, and object verification decodes emitted words through catalog-backed opcode recognition while validating relocation families.
- The compile path now lowers `movz`, `add-shifted-register`, and `ret` from allocated physical registers with byte provenance preserved from the machine instruction origin. The end-to-end ADD regression test proves the emitted bytes contain the ADD word, not a silent NOP fallback.
- A second independent review found and the implementation corrected additional integration gaps: non-leaf public calls now produce non-leaf unwind metadata, final object symbols use layout fragment offsets, backend fact import performs per-family payload validation, object verification checks unwind/literal/veneer/relocation consistency, and debug artifact requests surface allocation/frame summaries.
- A later review found the non-leaf byte stream still needed to match its frame metadata. Public-call functions now emit a concrete `sp` adjustment, `x30` save/restore, the `bl` relocation before the final return, and matching `add/sub/ldr/str` encoders and verifier decode coverage.
- Backend layout and object verification now consume the authenticated target catalogs supplied to `compileAArch64Object`; the RPi5 catalogs remain only as explicit defaults for standalone helper calls and tests.
- Allocation repair requests are no longer advisory-only: spillable unallocated intervals become deterministic spill repair work items, allocation verification checks interval coverage and physical-register overlap, and physical instruction IR preserves machine order instead of sorting by stable key.
- Machine ABI metadata is projected into public ABI classification rather than replaced with empty parameter/return lists. The ABI classifier also caps public return GPR/vector budgets and recognizes homogeneous vector aggregates.
- Expanded conditional/test branch layouts now retain a backend-owned `branch26` relocation at the expanded branch patch site, and object verification skips declared literal-pool data when decoding instruction words.
- OptIR and backend fact transfer now visibly share the `CompilerFactExtension`, `FactVerifierKey`, and `FactTransferBehavior` vocabulary while keeping OptIR-facing error compatibility.
- The final independent review found and the implementation corrected four last correctness gaps: facts no longer synthesize backend code without machine IR, packet-loop provenance fixtures now use real machine instructions, object verification checks relocation patch ownership against the decoded opcode/catalog bit range, object-module fingerprints include emitted bytes and record payloads, and encoding catalog normalization sorts entries by opcode consistently.
- A subsequent review found six remaining backend-depth gaps and the implementation corrected them: the RPi5 backend catalog now carries the full Task 26A closed opcode inventory with known-byte fixture evidence for every row; backend pseudos only expand to closed-inventory opcodes with executable encoders; public ABI overflow returns use the AAPCS64 `x8` indirect result path instead of stack return locations; fact re-keying covers every current machine fact subject kind with explicit stale-subject policy; liveness is CFG dataflow-based and interference now reports alias-expanded physical clobbers; spill repair drafts are consumed by final lowering through reserved scratch reload/store sequences; and the post-RA scheduler is a deterministic dependency-island scheduler rather than an identity sort.
- The executable encoder remains intentionally narrower than the authored catalog where machine-IR opcodes are authenticated but not yet emitted by final object lowering. Encoding attempts for authenticated-but-unimplemented opcodes fail closed with `encoding:unsupported-family`, while all opcodes emitted by `physical-instruction-ir.ts`, pseudo expansion, prologue/epilogue, and layout finalization have direct encoders or explicit diagnostics.
- The final review loop corrected the remaining acceptance gaps: layout-and-encode now iterates a real monotone fixed point and has a cascading branch-widening regression; object verification checks relocation patches against fragments, branch/page-offset range policy, literal-pool bytes/overlap/secret provenance, veneer byte encoding/range assumptions, symbol placement, and exact byte-provenance partitioning; target authentication requires the fixed Task 26A relocation mapping set; wipe-on-spill facts flow into spill repair/frame layout/fact spending; and end-to-end tests now include compile-level no-spill and relocation-mapping failures.
- A fresh no-context review then found five integration gaps and the implementation corrected them: frame plans now emit concrete stack-adjust/save/restore/wipe bytes for both non-leaf and leaf-spill frames; closed-image private conventions preserve argument/result/clobber/pinning metadata and are consumed by compile; reconciled call and machine-liveness clobbers are treated as conservative physical allocation constraints; backend-owned veneers retarget the original branch to a generated local veneer symbol and add a relocation from the veneer body to the original target; and final object verification receives a non-empty security-conservation input derived from facts, allocation placements, spill slots, and unambiguous wipe events.
- A final no-context review found five last edge-case gaps and the implementation corrected them: closed-image call-plan metadata no longer synthesizes executable `bl` instructions without matching machine IR; layout encoding returns diagnostics instead of constructing invalid zero-byte fragments and records provenance for alignment padding; conditional branch widening preserves the original condition semantics and unsupported test-branch widening fails closed; global symbol fact subjects are indexed by stable symbol id instead of object stringification; and object verification decodes instruction ownership through authenticated catalog word patterns instead of a hard-coded opcode subset.
- A late thermo-nuclear maintainability pass and no-context review found five additional hardening gaps and the implementation corrected them: the compile API was split into focused verification, lowering, frame-instruction, pipeline, and object-assembly modules; input-contract verification now dry-runs fact import and indexes machine functions, blocks, instructions, registers, frame objects, symbols, relocations, call sites, machine edges, target declarations, and dropped facts; packet-loop acceptance uses real load/endian/barrier instructions with proof-specific fact-to-byte provenance; wipe-on-spill obligations are exit-scoped so multi-function objects do not cross-check unrelated exits; and all opcode families emitted by backend lowering now have executable encoders, catalog patterns, and representative known-byte tests.
- The final signoff review found five production-hardening misses and the implementation corrected them: object verification now treats authenticated alignment-padding provenance as non-instruction bytes; backend-owned veneer requests must include matching backend-owned veneer-site metadata before layout can return `ok`; frame-object pseudos fail closed on missing frame slots instead of defaulting to `[sp,#0]`; registered backend fact families use per-family payload schemas including typed memory-order region facts; and deterministic test stage overrides can now force a stage failure through the public compile pipeline.
- A subsequent no-context review found three target/object hardening misses and the implementation corrected them: target-surface authentication now re-authenticates caller-provided encoding catalogs, normalizes catalog word patterns, and rebuilds lookup functions from authenticated entries; memory encoders reject ZR and non-address registers as memory bases while preserving legal SP stack operands; and external public callees are materialized as `.extern` symbols only when emitted object relocations actually target them.
- A final review pass found four backend integration gaps and the implementation corrected them: public object compilation now lowers local `b`, `b-cond`, `cbz`, `cbnz`, `tbz`, and immediate add/sub machine opcodes with sparse branch-target symbols; backend facts cannot self-authorize target declarations unless the preserved fact set issued them; object-level security verification receives secret branch, table, helper, no-spill, wipe, and constant-time-helper obligations; and byte provenance only attaches facts to matching provenance records instead of falling back to the first byte.
- The next fresh signoff review found four final AC gaps and the implementation corrected them: framed functions now splice restore/return epilogues at every machine return site instead of falling through to one trailing epilogue; allocation pools come from the authenticated register model's allocatable records; fact-to-byte provenance uses exact typed-subject matching rather than substring matching; and verifier traces are emitted only when explicitly requested in debug artifacts.
- The subsequent fresh signoff review found four hardening gaps and the implementation corrected them: target authentication now rebuilds register, relocation, unwind, and veneer lookup callbacks from normalized catalog records instead of trusting caller-supplied functions; compile-time interference receives real physical alias pairs from the authenticated register model; fact-to-byte provenance only permits exact typed-subject matches plus the explicit `region:<source>` bridge; and empty object sections no longer synthesize invalid zero-length byte-provenance records.
- Another fresh signoff review found four remaining allocation/finalization gaps and the implementation corrected them: allocator and allocation verification now reject aliasing physical-register overlap using storage-view aliases; move resolution handles deterministic acyclic chains and cycles of arbitrary length with legal temporaries or security-checked memory fallback; the compile pipeline routes real entry copy webs through move resolution and materializes encodable copy moves; and the post-RA scheduler now models call/exit, ordered-memory, FPCR/FPSR, and vector-state dependencies in addition to existing barrier/NZCV/relocation/secret-region constraints.
- The latest fresh signoff review found three remaining public-surface/backend-layout gaps and the implementation corrected them: compare-zero and test-bit branch widening now emits inverted skip branches plus backend-owned `branch26` relocations for `cbz`, `cbnz`, `tbz`, and `tbnz`; the RPi5 public ABI register model no longer marks callee-saved `x19`-`x28` as caller-saved clobbers; and deterministic public stage overrides now cover the final `debug-artifact-collection` and `end-to-end-stage-wiring` stages before a successful compile return.
- The subsequent fresh signoff review found three remaining layout/fact/scheduler gaps and the implementation corrected them: literal-pool islands now plan against rendered section ends, carry per-user reach metadata into object modules, and are verified against final offsets; object-stage fact subjects supplied through public compile input are rejected as stale before object output; and optional post-RA load-latency scheduling now preserves physical register RAW/WAR/WAW dependencies with compile-populated register/NZCV metadata.
- The lowering implementation was split below the 1k-line maintainability threshold: `machine-lowering.ts` now owns orchestration and opcode lowering, `machine-lowering-helpers.ts` owns shared operand, branch-symbol, and diagnostic helpers, and `machine-lowering-repairs.ts` owns spill/reload/remat repair lowering.
- The function pipeline was kept below the 1k-line maintainability threshold by extracting storage-alias and entry-copy resolution into `function-copy-resolution.ts`.
- A fresh signoff pass found four remaining backend-hardening gaps and the implementation corrected them: cycle move temporaries now exclude all active copy sources and destinations; expanded conditional/compare/test branch sequences fail closed when their backend-owned `branch26` leg exceeds reach; wipe-on-spill obligations are projected to every observable `ret`, `trap`, and register-branch tail exit and the emitted code inserts wipe/cleanup preludes before trap and tail exits; and object-level wipe labels now come from imported wipe-on-spill facts plus actual spill-slot placements rather than from existing wipe events.
- The backend maintainability split was completed under the 1k-line threshold: `function-pipeline.ts` now delegates security projection to `function-security-projection.ts`, and `layout-encode-fixed-point.ts` now delegates widened branch encoding to `layout-branch-expansion.ts`.
- The next fresh signoff pass found three deterministic authority gaps and the implementation corrected them: input-contract verification now rejects duplicate machine identities even when no facts reference them; closed-image plan verification rejects duplicate plan records even when the authority fingerprint is recomputed; and backend output paths use code-unit string ordering rather than locale-dependent ordering.
- The following fresh signoff pass found five final hardening gaps and the implementation corrected them: authenticated backend catalog fingerprints are now content-addressed from normalized catalog records instead of caller-provided strings; `blr` indirect calls are indexed as call-site subjects and treated as public ABI/frame call boundaries; live-through-call liveness intervals preserve split cut points and clobber metadata; branch26/branch19/branch14 range checks use the A64 signed scaled asymmetric positive boundary; and rewrite transaction test builders were moved out of runtime exports.
- The next clean-room review found four last authority/determinism issues and the implementation corrected them: encoding catalog authentication now rejects duplicate opcode forms even under distinct stable keys; backend target authentication rejects encoding relocation holes whose families are not authorized by the relocation catalog; closed-image plan normalization no longer throws on conflicting duplicate private conventions and leaves deterministic rejection to the verifier; and preserved machine facts now use code-unit ordering.
- The latest clean-room review found five final ABI/object authority gaps and the implementation corrected them: public ABI fixed-register metadata now assigns the requested public GPR/vector slot while advancing later argument cursors; public vector call clobbers are preserved through call-boundary reconciliation, allocation unavailability, and debug artifacts; add/sub immediate encoding rejects zero-register aliases while preserving legal `sp`; authored catalogs reject emitted opcodes without verifier word patterns; and backend-owned veneer range verification now uses the asymmetric signed scaled branch26 reach.
- The final fresh clean-room pass found six remaining edge-case gaps and the implementation corrected them: fact-spending records now family-qualify stable keys, input-contract verification rejects ambiguous `memoryOperand` fact subjects, object verification treats `security-and-secret-lifetime` literal-pool provenance as secret, target authentication rejects catalogs missing opcodes emitted by the backend, large aggregate public parameters use AAPCS64 indirect pointer slots, and public call clobbers now become interval-specific physical interferences instead of shrinking allocation pools function-wide.
- The subsequent fresh signoff pass found three final hardening issues and the implementation corrected them: target authentication rejects duplicate physical-register and relocation-family identities before rebuilding lookup callbacks, malformed fact verifier keys now return input-contract diagnostics instead of throwing through the public compile API, and AArch64 fact adapter registry ordering now uses code-unit ordering rather than locale-sensitive comparison.
- The next fresh signoff pass found two relocation/encoding boundary issues and the implementation corrected them: object verification now validates page-offset relocations against the encoded low-12 page offset rather than the absolute symbol offset, and `add-pageoff` rejects stack/zero-register aliases instead of silently encoding register number 31.
- The latest fresh signoff pass found two finalization integration gaps and the implementation corrected them: fact-backed constant rematerialization authorities now flow through public compile into allocation repair, carry a concrete constant recipe, elide only exact pure `movz` definitions, and materialize at unallocated use sites instead of allocating spill slots; and lowering, move resolution, frame finalization, and pseudo expansion now commit emitted replacements through a runtime backend rewrite-transaction bridge while preserving typed instruction objects for encoding.
- The next fresh signoff pass found four final AC/hardening gaps and the implementation corrected them: object verification now decodes page-offset relocation fields and rejects stale low-12 encodings; branch relaxation initial site estimates treat `label` instructions as zero-byte entries like the renderer; input-contract verification indexes `memoryOperand` fact subjects only for address operands; and the allocator now performs deterministic cut-point splitting before falling back to spill repair when split children can relieve register pressure.
- The following fresh signoff pass found two deterministic diagnostic gaps and the implementation corrected them: layout-and-encode now reports zero-byte fragments such as label-only fragments as diagnostics instead of throwing object-construction errors, and public stage override validation rejects unknown runtime override results while empty-program compilation honors forced per-function stage overrides before returning an empty object.
- Tests are the executable acceptance ledger for the implemented task families: shared fact core, backend target authentication, closed-image plan verification, fact import/query, rewrite transfer, security conservation, physical registers, ABI/public-private call boundaries, allocation/spill/move/frame/unwind/finalization/scheduling, encoding/catalog/relocations/layout/object verification, debug artifacts, and end-to-end object compilation.

Verification evidence at implementation time:

```bash
PATH="$HOME/.bun/bin:/opt/homebrew/bin:$PATH" bun test ./tests/unit/shared/facts ./tests/unit/target/aarch64/backend ./tests/integration/target/aarch64/backend-object.test.ts
./node_modules/.bin/tsc --noEmit
PATH="$HOME/.bun/bin:/opt/homebrew/bin:$PATH" bun run agent:check
```

Latest full gate after the no-context review fixes:
...
The final handoff gate remains `bun run agent:check` per `AGENTS.md`.
