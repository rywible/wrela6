...
import { MatchStatementView, BlockView } from "../../frontend/ast/statement-views";
...
import { SourceSpan, SourceText, RedNode } from "../../frontend";
...
function hasTypeParameters(decl: DeclarationView): decl is DeclarationView & HasTypeParameters {
  return "typeParameters" in decl && typeof (decl as any).typeParameters === "function";
}

function hasFields(decl: DeclarationView): decl is DeclarationView & HasFields {
  return "fields" in decl && typeof (decl as any).fields === "function";
}

function hasMemberFunctions(decl: DeclarationView): decl is DeclarationView & HasMemberFunctions {
  return "memberFunctions" in decl && typeof (decl as any).memberFunctions === "function";
}
...
function walkPatternsInBlock(func: FunctionDeclarationView, context: TypeResolutionContext): void {
...
function resolvePattern(pattern: PatternView, context: TypeResolutionContext): void {
  ...
  if (segTexts.length === 1) {
    // Simple pattern name - check if it resolves as a value, otherwise treat as binder
    ...
    // Bare simple pattern that doesn't resolve -> binder, no diagnostic
    return;
  }

  // Qualified pattern like PacketKind.ping
...
function resolveQualifiedPattern(
  ...
  if (prefixResult.kind === "noModulePrefix") {
    ...
    if (ownerResult.kind === "resolved") {
      ...
      if (ownerRef.kind === "item" || ownerRef.kind === "type") {
        const ownerItemId = "itemId" in ownerRef ? ((ownerRef as any).itemId as ItemId) : undefined;
        if (ownerItemId !== undefined) {
          ...
          for (const memberName of memberSegments) {
            const memberResult = context.memberNamespace.resolveMember({
              ownerItemId,
              name: memberName,
            });
            if (memberResult.kind === "resolved") {
              const memberSpan = getSegmentSpan(segments, segTexts.indexOf(memberName));
    ...
    // No resolution -> binder, no diagnostic
    return;
  }
  ...
  // Resolved module prefix - look up item in target module
