...
export function createOptIrFreshIdAllocator(input: {
  ...
  let nextFunctionId =
    Math.max(
      0,
      ...input.program.functions.entries().map((function_) => Number(function_.functionId)),
    ) + 1;
  let nextBlockId =
    Math.max(
      0,
  ...
  let nextRegionId =
    Math.max(0, ...input.program.regions.entries().map((region) => Number(region.regionId))) + 1;

  return {
    functionId() {
      return optIrFunctionId(nextFunctionId++);
    },
    blockId() {
    ...
    regionId() {
      return optIrRegionId(nextRegionId++);
    },
  };
}
